...
const float TURB_LOW_THRESHOLD = 100.0f;
...
void OnDataRecv(const uint8_t* mac, const uint8_t* data, int len) {
    ...
    if (s.startsWith("LOG:")) {
        ...
        if (current_mode == "LEARNING" && sample_idx > 5) {
            ...
            th_dry_run    = baseline_mean - (3.0f * baseline_std);
            if (th_dry_run > baseline_mean - 15.0f) th_dry_run = baseline_mean - 15.0f;
...
void applyClearHalt() {
    ...
    current_mode        = "IDLE";
    stall_confirm = volt_confirm = dry_confirm = temp_confirm = turb_confirm = 0;
    targetPumpOn        = false;
    targetHalted        = false;
...
// INA (Target)
    ...
    Serial.println("\n== FLOAT Observer Node ==");
    Serial.println("SPC: Shewhart(stall+dry) + CUSUM(degrade) | adaptive TEMP | battery cutoff(volt) | turbidity(warn) | VOLT/TEMP/TURB = warning");
}
void loop() {
    server.handleClient();
#if MQTT_ENABLED
...
// Arduino Uno 
    ...
    if (current_mode == "MONITORING" && is_calibrated) {
        ...
        // DRY_RUN
        bool dry_flag   = (th_dry_run > 0.1f && ewma_current < th_dry_run && ewma_current > 20.0f && !volt_flag);
        // TEMPERATURE
        ...
        // TURBIDITY
        bool turb_flag  = (last_turb < TURB_LOW_THRESHOLD);
        ...
        if (turb_flag)  turb_confirm++;  else turb_confirm = 0;
        // Degradation
        ...
        bool anomaly = (stall_confirm >= CONFIRM_NEEDED) ||
                       ...
                       (turb_confirm >= CONFIRM_NEEDED);
        Serial.printf("   [MON] I_raw=%.1f  I_ewma=%.1f  V=%.2f  T=%.1f  turb=%.1f"
                      "  Tthr[H%.1f L%.1f]  [S:%d V:%d D:%d T:%d Tb:%d]%s%s%s%s%s%s\n",
                      last_current, ewma_current, last_voltage, last_temp_c, last_turb,
                      th_temp_high, th_temp_low,
                      stall_confirm, volt_confirm, dry_confirm, temp_confirm, turb_confirm,
                      stall_flag      ? " STALL"     : "",
                      volt_flag       ? " VOLT-LOW"  : "",
                      dry_flag        ? " DRY-RUN"   : "",
        ...
        if (anomaly) {
            ...
            else if (temp_confirm >= CONFIRM_NEEDED)      anomaly_reason = temp_high_flag ? "TEMP_TOO_HIGH" : "TEMP_TOO_LOW";
            else                                          anomaly_reason = "TURBIDITY_LOW";
            Serial.printf("\n[!!!] ANOMALY CONFIRMED: %s\n", anomaly_reason.c_str());
            bool isHalt = (anomaly_reason == "MOTOR_STALL" || anomaly_reason == "DRY_RUN");
            // Evaluation (halt classes only)
            if (isHalt && eval_truth >= 0 && !eval_cycle_recorded) {
            ...
            // MOTOR_STALL / DRY_RUN -> halt the pump; VOLTAGE_DROP / TEMP / TURBIDITY -> warning only
