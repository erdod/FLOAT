...
function showToast(type, reason){
  const labels={
    ...
    TURBIDITY_LOW:'Turbidity low \u2014 water dirty',
...
const EVAL_LABELS=['Normal','Stall','Dry-run'];
const EVAL_SHORT =['NOR','STA','DRY'];
let evalTruth=-1;
...
function markTruth(){
  for(let k=0;k<EVAL_LABELS.length;k++){ const b=document.getElementById('ev-'+k); if(b) b.className='mbtn'+(k===evalTruth?' act':''); }
}
...
function renderEval(d){
  const m=d.m, T=EVAL_SHORT.length;
  let tot=0,diag=0;
  const rowSum=Array(T).fill(0), colSum=Array(T).fill(0), diagK=Array(T).fill(0);
  for(let i=0;i<T;i++)for(let j=0;j<T;j++){
    const v=m[i*T+j]; tot+=v; rowSum[i]+=v; colSum[j]+=v;
    if(i===j){ diag+=v; diagK[i]=v; }
  }
  let fprSum=0,fprN=0,fnrSum=0,fnrN=0;
  for(let k=0;k<T;k++){
    const TP=diagK[k], FN=rowSum[k]-TP, FP=colSum[k]-TP, negK=tot-rowSum[k];
    if(rowSum[k]>0){ fnrSum+=FN/rowSum[k]; fnrN++; }
    if(negK>0){ fprSum+=FP/negK; fprN++; }
  }
  const acc=tot?(100*diag/tot):0;
  const fprMacro=fprN?(100*fprSum/fprN):0;
  const fnrMacro=fnrN?(100*fnrSum/fnrN):0;
  const st=document.getElementById('eval-stats');
  const cur=(d.truth>=0)?('Active truth: <b>'+EVAL_LABELS[d.truth]+'</b>'):'Labelling off';
  st.innerHTML=cur+' &middot; samples: <b>'+d.count+'</b> &middot; accuracy: <b>'+acc.toFixed(1)+
    '%</b> &middot; macro FPR: <b>'+fprMacro.toFixed(1)+'%</b> &middot; macro FNR: <b>'+fnrMacro.toFixed(1)+'%</b>';
